...
| [x] | M2-S04 | Create `room` and `room_block` with branch/type/booking FKs, nullable current-stay pointer, branch-scoped room number uniqueness and valid block intervals. Done when invalid FK, duplicate and date tests pass. Depends on M2-S02/S03 and Member 1's branch. |
| [x] | M2-S05 | Add the approved `booking_room_assignment` migration: UUIDv7 PK, booking/room FKs, assignment timestamps and at most one open assignment per booking. Done when sequential history and duplicate-open tests pass. Depends on M2-S03/S04. |
| [x] | M2-S06 | Add database-side same-room/date overlap protection and pointer consistency guards with Members 3/4's transition contract. Done when direct invalid writes and two simultaneous overlapping bookings fail, while adjacent stays pass. Depends on M2-S05. |
...
M2-S03 (19 September 2026): [Booking migration](../backend/migrations/m2_002_booking.sql) creates the approved booking channel/status enums, `booking` and `booking_status_history`, including UUIDv7, reference/date/count/rate/time, transition and restricted guest/actor/history FK constraints. `npm run test:m2-booking --workspace backend` passed in a rolled-back PostgreSQL 18 scratch schema using minimal Member 1 parent fixtures; `npm run test:m2-catalogue --workspace backend` and `npm run build:backend` also passed. The real ordered chain still depends on Member 1's `guest`/`user_account` migrations. No booking API/write path was added; M2-S06 now supplies its database guards, while M2-S09/M2-S10 remain the later availability/create paths.

M2-S04 (19 September 2026): [Room inventory migration](../backend/migrations/m2_003_room_inventory.sql) creates the approved room-status enum, `room` and `room_block` with UUIDv7 IDs, required branch/type/actor links, the nullable current-stay booking pointer, branch-scoped room-number uniqueness and valid block intervals. `npm run test:m2-rooms --workspace backend` passed after applying M2-S02 through M2-S04 in one rolled-back PostgreSQL 18 scratch schema; the M2-S02/M2-S03 regressions and `npm run build:backend` also passed. The production chain still needs Member 1's `branch`/`user_account` migrations. M2-S05 and M2-S06 now supply assignment history, overlap protection and pointer-state guards.

M2-S05 (20 September 2026): [Assignment migration](../backend/migrations/m2_004_booking_room_assignment.sql) creates the approved booking-room history table with UUIDv7, required booking/room FKs, ordered assignment timestamps and a partial unique index allowing at most one open row per booking. `npm run test:m2-assignments --workspace backend` passed sequential history, invalid-row and two-session simultaneous-open tests; M2-S02 through M2-S04 regressions and `npm run build:backend` also passed. M2-S06 now supplies the same-room stay-date, active-booking lifecycle and `room.booking_id` consistency guards.

M2-S06 (20 September 2026): [Guard migration](../backend/migrations/m2_005_assignment_guards.sql) serializes room-affecting writes with transaction advisory locks, rejects overlapping half-open stay ranges for active bookings, and uses deferred constraint triggers to validate active/terminal assignment counts and the CHECKED_IN/OCCUPIED current-room pointer at transaction end. [Focused tests](../backend/tests/m2Guards.test.cjs) passed direct invalid writes, adjacent stays, valid coordinated check-in/checkout/cancellation, and a two-session overlap race; all M2-S02 through M2-S05 regressions and `npm run build:backend` also passed. M2-S09/M2-S10 still own availability and booking APIs, and Members 3/4 still own their transition procedures and histories.
