...
**Approved extension to the final ER (not one of its 21 transcribed entities):** Add `booking_room_assignment` with `assignment_id UUIDv7` as primary key, `booking_id UUIDv7 → booking.booking_id`, `room_id UUIDv7 → room.room_id`, `assigned_at timestamp` and nullable `unassigned_at timestamp`. `assigned_at`/`unassigned_at` preserve each assignment period; ending an assignment never deletes its row. At most one assignment per booking may have `unassigned_at IS NULL`. A BOOKED or CHECKED_IN booking must have exactly one such open assignment; CHECKED_OUT, CANCELLED and NO_SHOW bookings retain closed assignment history. A reassignment closes the old row and inserts the new one within one transaction. The booking's branch is the assigned room's branch. This is an approved conceptual extension, not a claim that the supplied diagram already contains these attributes. M2-S05 implements and tests the structural table, timestamp ordering and concurrency-safe one-open-assignment rule. M2-S06 implements and tests same-room active-date overlap, active/terminal assignment counts and current-pointer consistency at transaction end.
...
**M2-S01 working handoff (18 September 2026):** Imandi reports that Members 1, 3 and 4 agree to use the reservation ownership and transaction boundaries recorded in [the Member 2 reservation contract](member_tasks/m2_s01_reservation_contract.md) and cross-referenced in their task plans. Member 2 owns booking/assignment schema and reservation writes; Member 3 owns check-in and room-status history; Member 4 owns checkout, cancellation and no-show orchestration; Member 1 supplies identity, authorization, actor and audit contracts. Shared actor FKs target `user_account.user_id`, including online actions, with a dedicated non-login account for system actions and no hard deletion of referenced accounts. New JSON routes use `/api/*`; existing `GET /rooms` remains a compatibility alias during migration. Other owners' Appendix C decisions remain open.

**M2-S06 implementation (20 September 2026):** Room-affecting writes use transaction-level PostgreSQL advisory locks derived from room UUIDs; multi-room operations acquire distinct IDs in UUID order through `m2_lock_room_ids(uuid[])`. Open assignments for BOOKED/CHECKED_IN bookings are compared with half-open `daterange` overlap, so adjacent stays remain valid. Deferred constraint triggers validate the final transaction state: active bookings have one open assignment, terminal bookings have none, and only a CHECKED_IN booking may own its assigned OCCUPIED room pointer. This permits coordinated multi-table transitions inside one transaction while rejecting inconsistent commits and serializing simultaneous conflicts. The transition procedures, history writers, authorization and application retry/error mapping remain with their listed owners.
