...
Before database or member-owned feature work, read `README.md` for the approved React/Vite + Express/Node stack, `SkyNest_HRGSMS_SRS_v1.0.md` for the current Version 1.2 draft requirements (especially §4 and §6.1), `member_summary_table.md` for ownership/handoffs, and the relevant file in `member_tasks/` for a one-commit-sized work plan. Then recheck current schema/code and affected teammates' confirmed contracts. The final ER's 21-entity attribute/type inventory is SRS Table 40; the separately approved `booking_room_assignment` extension preserves history while nullable `room.booking_id` points only to the currently checked-in booking. Online guests use `guest_account` and can access only their own records. Appendix C still leaves enum labels, UUIDv7 generation, decimal scale, actor FK targets and other mappings open. Do not silently decide those details or rewrite existing migrations/code unless the current task explicitly requires it.
