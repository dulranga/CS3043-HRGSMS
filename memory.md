...
| Reservation database guards — 20 September 2026 | M2-S06 serializes room-affecting writes with transaction advisory locks derived from room UUIDs and requires UUID order for multi-room locks. Active same-room stays use half-open date overlap; deferred checks enforce active/terminal open-assignment counts and the CHECKED_IN/OCCUPIED current pointer at transaction end. See SRS §6.1.7–§6.1.8 and the M2-S01 contract. |
